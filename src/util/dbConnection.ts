...
export async function saveProductToDb(data: ProductDB, productDB: D1Database): Promise<void> {
	try {
		await productDB
			.prepare(`INSERT into product (productId,title,handle,published_at,created_at,updated_at,price,available,shopifyStore	
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
			.bind(data.productId, data.title, data.handle, data.published_at, data.created_at, data.updated_at, data.price, data.available, data.shopifyStore)
			.run();
	} catch (e) {
...
export async function updateProductById(productId: string, data: Partial<ProductDB>, productDB: D1Database): Promise<void> {
	try {
		await productDB
			.prepare(`UPDATE product SET title = ?, handle = ?, published_at = ?, created_at = ?, updated_at = ?, price = ?, available = ?, shopifyStore = ? WHERE productId = ?`)
			.bind(data.title, data.handle, data.published_at, data.created_at, data.updated_at, data.price, data.available, data.shopifyStore, productId)
			.run();
	} catch (e) {
